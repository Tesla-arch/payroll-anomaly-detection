...
- `POST /auth/login/staff` `{ employee_id, email }` — Teachers and Accountants (staff ID + employment email; no password)
...
- `POST /payrolls/{id}/exclude` — drop a staff line from a **draft** run (HR / Headteacher). Open flags on that line are closed.
...
`broadcast` emails every active parent. `meeting` requires `meeting_at`. Mail uses the parent user’s registered email and is sent From `MAIL_FROM_ADDRESS` (school account `victorqwao855@gmail.com` via Gmail SMTP). WhatsApp messages send from `WHATSAPP_FROM_NUMBER` (default `0591723646`). `GET /parents` reports `stats.mail_live` and `stats.whatsapp_live`. Live WhatsApp needs `WHATSAPP_DRIVER=green_api` (scan QR with the school phone) or `cloud` with a Meta token.
