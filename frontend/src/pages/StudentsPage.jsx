      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="table">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="cursor-pointer hover:bg-emerald-50/60" onClick={() => navigate(`/students/${row.id}/assessment`)}>
                  <td className="font-medium">{row.admission_number}</td>
                  <td>{row.display_name || `${row.first_name} ${row.last_name}`}</td>
                  <td>
                    {row.school_class ? (
                      <span>
                  ...
                  <td className="capitalize">{row.status || 'active'}</td>
                  <td className="space-x-3 text-right" onClick={(event) => event.stopPropagation()}>
